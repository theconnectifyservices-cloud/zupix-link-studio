...
import {
  BACKGROUND_PATTERNS, DEFAULT_THEME, DEFAULT_PROFILE, DEFAULT_BUTTONS,
  DEFAULT_BACKGROUND, GOOGLE_FONTS, THEME_PRESETS, ensureGoogleFont,
  type BackgroundKind, type ButtonShapeId, type ButtonVariantId,
  type IconPositionId, type PageTheme, type ThemeMode, type ThemePresetId,
} from "../theme";
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value="colors" className="mt-3 space-y-3">
          ...
          <ColorField label="Background" value={theme.colors.background}
            brands={brands} onSaveBrand={addBrand}
            onChange={(v) => patchColors({ background: v, backgroundSolid: extractSolid(v) ?? theme.colors.backgroundSolid })} />
          <ColorField label="Surface" value={theme.colors.surface} brands={brands}
            onSaveBrand={addBrand} onChange={(v) => patchColors({ surface: v })} />
          <ColorField label="Card" value={theme.colors.card} brands={brands}
            onSaveBrand={addBrand} onChange={(v) => patchColors({ card: v })} />
          <ColorField label="Text" value={theme.colors.text} brands={brands}
            onSaveBrand={addBrand} onChange={(v) => patchColors({ text: v })} />
          <ColorField label="Muted text" value={theme.colors.textMuted} brands={brands}
            onSaveBrand={addBrand} onChange={(v) => patchColors({ textMuted: v })} />
          <ColorField label="Border" value={theme.colors.border} brands={brands}
            onSaveBrand={addBrand} onChange={(v) => patchColors({ border: v })} />
          <ColorField label="Primary" value={theme.colors.primary} brands={brands}
            onSaveBrand={addBrand} onChange={(v) => patchColors({ primary: v })} />
          <ColorField label="Primary text" value={theme.colors.primaryText} brands={brands}
            onSaveBrand={addBrand} onChange={(v) => patchColors({ primaryText: v })} />
          <ColorField label="Secondary" value={theme.colors.secondary} brands={brands}
            onSaveBrand={addBrand} onChange={(v) => patchColors({ secondary: v })} />
          <ColorField label="Accent" value={theme.colors.accent} brands={brands}
            onSaveBrand={addBrand} onChange={(v) => patchColors({ accent: v })} />
          <ColorField label="Icon" value={theme.colors.icon ?? theme.colors.text} brands={brands}
            onSaveBrand={addBrand} onChange={(v) => patchColors({ icon: v })} />
          <ColorField label="Link" value={theme.colors.link ?? theme.colors.accent} brands={brands}
            onSaveBrand={addBrand} onChange={(v) => patchColors({ link: v })} />
        ...
        <TabsContent value="type" className="mt-3 space-y-3">
          ...
          <FontSelect label="Body font" value={theme.typography.fontFamily}
            onChange={(v) => { patchType({ fontFamily: v }); rememberGoogle(patch, theme, v); }} />
          <FontSelect label="Heading font" value={theme.typography.headingFamily}
            onChange={(v) => { patchType({ headingFamily: v }); rememberGoogle(patch, theme, v); }} />
          <FontSelect label="Button font" value={theme.typography.buttonFamily}
            onChange={(v) => { patchType({ buttonFamily: v }); rememberGoogle(patch, theme, v); }} />
          <NumField label="Base size" min={12} max={20} step={1}
            value={theme.typography.baseSize} suffix="px"
            onChange={(v) => patchType({ baseSize: v })} />
          <NumField label="Button size" min={11} max={20} step={1}
            value={theme.typography.buttonSize ?? 14} suffix="px"
          ...
          <Field label="Text transform">
            <Select value={theme.typography.textTransform ?? "none"}
              onValueChange={(v) => patchType({ textTransform: v as PageTheme["typography"]["textTransform"] })}>
        ...
        <TabsContent value="buttons" className="mt-3 space-y-3">
          ...
          <Field label="Variant">
            <Select value={buttons.variant} onValueChange={(v) => patchButtons({ variant: v as ButtonVariantId })}>
              <SelectTrigger className="h-8"><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <Field label="Shape">
            <Select value={buttons.shape} onValueChange={(v) => patchButtons({ shape: v as ButtonShapeId })}>
              <SelectTrigger className="h-8"><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <NumField label="Height" min={32} max={80} step={1} value={buttons.height} suffix="px"
            onChange={(v) => patchButtons({ height: v })} />
          <NumField label="Radius" min={0} max={40} step={1} value={buttons.radius} suffix="px"
            onChange={(v) => patchButtons({ radius: v })} />
          <NumField label="Padding X" min={8} max={40} step={1} value={buttons.paddingX} suffix="px"
            onChange={(v) => patchButtons({ paddingX: v })} />
          <NumField label="Border width" min={0} max={4} step={1} value={buttons.border} suffix="px"
          ...
          <Field label="Icon position">
            <Select value={buttons.iconPosition}
          ...
          <NumField label="Icon size" min={12} max={28} step={1} value={buttons.iconSize} suffix="px"
            onChange={(v) => patchButtons({ iconSize: v })} />
          <Field label="Alignment">
            <Select value={buttons.align} onValueChange={(v) => patchButtons({ align: v as PageTheme["buttons"] extends infer B ? B extends { align: infer A } ? A : never : never })}>
              <SelectTrigger className="h-8"><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectItem value="left">Left</SelectItem>
        ...
        <TabsContent value="bg" className="mt-3 space-y-3">
          ...
          {(bg.kind === "color" || bg.kind === "gradient" || bg.kind === "glass") && (
            <ColorField label="Background color" value={theme.colors.background}
              brands={brands} onSaveBrand={addBrand}
              onChange={(v) => patchColors({ background: v, backgroundSolid: extractSolid(v) ?? theme.colors.backgroundSolid })} />
          )}
          ...
          {bg.kind === "image" && (
            <>
              <Field label="Image URL">
                <Input className="h-8" value={bg.imageUrl ?? ""}
                  onChange={(e) => patchBg({ imageUrl: e.target.value })}
                  placeholder="https://…" />
              </Field>
              <Field label="Size">
                <Select value={bg.size ?? "cover"} onValueChange={(v) => patchBg({ size: v as PageTheme["background"] extends infer B ? B extends { size?: infer S } ? S : never : never })}>
                  <SelectTrigger className="h-8"><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              <Field label="Position">
                <Select value={bg.position ?? "center"} onValueChange={(v) => patchBg({ position: v as PageTheme["background"] extends infer B ? B extends { position?: infer P } ? P : never : never })}>
                  <SelectTrigger className="h-8"><SelectValue /></SelectTrigger>
                  <SelectContent>
          ...
          {(bg.kind === "image" || bg.kind === "pattern" || bg.kind === "glass") && (
            <>
              <NumField label="Blur" min={0} max={40} step={1} value={bg.blur ?? 0} suffix="px"
                onChange={(v) => patchBg({ blur: v })} />
              <ColorField label="Overlay color" value={bg.overlay ?? "#000000"} brands={brands}
                onSaveBrand={addBrand} onChange={(v) => patchBg({ overlay: v })} />
              <NumField label="Overlay opacity" min={0} max={1} step={0.05}
                value={bg.overlayOpacity ?? 0} suffix=""
        ...
        <TabsContent value="card" className="mt-3 space-y-3">
          ...
          <NumField label="Card padding" min={0} max={40} step={1}
            value={theme.card.padding ?? 12} suffix="px"
            onChange={(v) => patchCard({ padding: v })} />
          <NumField label="Card margin" min={0} max={40} step={1}
            value={theme.card.margin ?? 0} suffix="px"
        ...
        <TabsContent value="profile" className="mt-3 space-y-3">
          ...
          <Field label="Avatar shape">
            <Select value={profile.avatarShape}
              onValueChange={(v) => patchProfile({ avatarShape: v as PageTheme["profile"] extends infer P ? P extends { avatarShape: infer S } ? S : never : never })}>
              <SelectTrigger className="h-8"><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <Field label="Avatar size">
            <Select value={profile.avatarSize}
              onValueChange={(v) => patchProfile({ avatarSize: v as PageTheme["profile"] extends infer P ? P extends { avatarSize: infer S } ? S : never : never })}>
              <SelectTrigger className="h-8"><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <NumField label="Avatar border" min={0} max={12} step={1}
            value={profile.avatarBorderWidth} suffix="px"
            onChange={(v) => patchProfile({ avatarBorderWidth: v })} />
          <ColorField label="Avatar border color" value={profile.avatarBorderColor}
            brands={brands} onSaveBrand={addBrand}
            onChange={(v) => patchProfile({ avatarBorderColor: v })} />
          <NumField label="Cover height" min={0} max={280} step={4}
            value={profile.coverHeight} suffix="px"
            onChange={(v) => patchProfile({ coverHeight: v })} />
          <NumField label="Name size" min={14} max={40} step={1}
            value={profile.nameSize} suffix="px"
            onChange={(v) => patchProfile({ nameSize: v })} />
          <WeightSelect label="Name weight" value={profile.nameWeight}
            onChange={(v) => patchProfile({ nameWeight: v as PageTheme["profile"] extends infer P ? P extends { nameWeight: infer W } ? W : never : never })}
            values={[400, 500, 600, 700, 800, 900]} />
          <NumField label="Bio size" min={10} max={20} step={1}
            value={profile.bioSize} suffix="px"
...
function ColorField({
  label, value, onChange, brands, onSaveBrand,
}: {
  label: string; value: string; onChange: (v: string) => void;
  brands: string[]; onSaveBrand: (hex: string) => void;
}) {
    ...
    <Field label={label}>
      <div className="flex items-center gap-2">
        ...
        <Button type="button" size="icon" variant="ghost"
          className="h-8 w-8 shrink-0" title="Save to brand colors"
        ...
        <div className="mt-1 flex flex-wrap gap-1">
          {brands.map((c) => (
            <button key={c} type="button" onClick={() => onChange(c)}
              ...
              style={{ background: c }} title={c} aria-label={`Use ${c}`} />
          ))}
        </div>
      )}
    </Field>
...
function FontSelect({
  ...
  const options = [
    ...SYSTEM_FONTS,
    ...GOOGLE_FONTS.map((g) => ({ label: g, value: `"${g}", ui-sans-serif, system-ui, sans-serif`, google: g })),
  ];
...
function rememberGoogle(patch: (p: Partial<PageTheme>) => void, theme: PageTheme, cssValue: string) {
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-2">
        {PALETTE_PRESETS.map((p) => (
          <button key={p.name} type="button" onClick={() => onPick(p)}
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-2">
        {GRADIENT_PRESETS.map((g) => (
          <button key={g.name} type="button" onClick={() => onPick(g)}
            className="h-10 rounded-md border transition-transform hover:scale-[1.02]"
            style={{ background: g.background }} title={g.name} aria-label={g.name} />
        ))}
      </div>
    </div>
  );
