...
import {
  Twitter, Instagram, Youtube, Linkedin, Github, Globe, Music2, Facebook,
  MessageCircle, Send, AtSign, Image as ImageIcon, BadgeCheck, MapPin, Link as LinkIcon,
  ChevronDown, Star, Download, Phone, Mail, MessageSquare, Sparkles, Play,
...
const SOCIAL_ICON: Record<SocialPlatform, LucideIcon> = {
  ...
  threads: AtSign, linkedin: Linkedin, pinterest: ImageIcon, telegram: Send,
  whatsapp: MessageCircle, github: Github, twitter: Twitter, website: Globe, custom: LinkIcon,
};
...
const ALIGN_WRAP = { left: "justify-start", center: "justify-center", right: "justify-end" } as const;
...
function renderInner(block: Block) {
  switch (block.type) {
    ...
    case "text":
      return (
        <p style={block.color ? { color: block.color } : undefined}
          className={cn("whitespace-pre-wrap text-foreground/80", FONT_SIZE[block.fontSize ?? "sm"],
    ...
    case "image": {
      if (!block.url) {
        return <div className="flex h-32 items-center justify-center rounded-md border border-dashed text-xs text-muted-foreground">Add an image URL</div>;
      }
      return (
        <img src={block.url} alt={block.alt ?? ""}
          className={cn("w-full", block.fit === "contain" ? "object-contain" : "object-cover",
      ...
      return (
        <div className="flex flex-wrap items-center justify-center gap-3">
          {block.links.map((l) => {
            ...
            return (
              <span key={l.id} className="grid h-9 w-9 place-items-center rounded-full bg-muted text-foreground"
    ...
    case "video": return <VideoRender block={block} />;
    case "gallery": return <GalleryRender block={block} />;
    case "socialFeed": return <SocialFeedRender block={block} />;
    case "testimonials": return <TestimonialsRender block={block} />;
    case "faq": return <FaqRender block={block} />;
    case "countdown": return <CountdownRender block={block} />;
    case "map": return <MapRender block={block} />;
    case "file": return <FileRender block={block} />;
    case "contact": return <ContactRender block={block} />;
...
function DividerRender({ block }: { block: Extract<Block, { type: "divider" }> }) {
  ...
  if (variant === "gradient") {
    ...
    return (
      <div className={spacing}
        style={{ height: block.thickness === "thick" ? 4 : block.thickness === "medium" ? 2 : 1,
  ...
  if (variant === "text") {
    return (
      <div className={cn("flex items-center gap-3 text-[11px] uppercase tracking-widest text-muted-foreground", spacing)}>
        <span className="h-px flex-1 bg-foreground/20" />
  ...
  const style = block.style === "dashed" ? "border-dashed" : block.style === "dotted" ? "border-dotted" : "border-solid";
  return (
    <hr className={cn("border-foreground/20", style, spacing,
...
function ButtonGroupRender({ block }: { block: ButtonGroupBlock }) {
  if (block.buttons.length === 0) {
    return <div className="rounded-md border border-dashed p-3 text-center text-xs text-muted-foreground">No buttons yet</div>;
  }
  const cls = block.layout === "horizontal" ? "flex flex-wrap gap-2"
    : block.layout === "grid" ? `grid gap-2 grid-cols-${block.columns ?? 2}`
    : "flex flex-col gap-2";
  return (
    <div className={cls}>
      {block.buttons.map((b) => {
        const v = b.style === "outline" ? "border border-foreground/30 text-foreground"
          : b.style === "soft" ? "bg-muted text-foreground" : "bg-foreground text-background";
        return (
          <div key={b.id}
            className={cn("rounded-full px-4 py-2.5 text-center text-sm font-medium", v,
...
function VideoRender({ block }: { block: VideoBlock }) {
  const roundedCls = block.rounded === "xl" ? "rounded-2xl" : block.rounded === "lg" ? "rounded-xl"
    : block.rounded === "md" ? "rounded-lg" : block.rounded === "sm" ? "rounded" : "rounded-none";
  if (!block.url) {
    return <div className={cn("flex aspect-video items-center justify-center border border-dashed text-xs text-muted-foreground", roundedCls)}>
      <div className="flex flex-col items-center gap-2"><Play className="h-6 w-6" /> Add a video URL</div>
    </div>;
  }
  if (block.provider === "mp4") {
    return <video src={block.url} controls muted={block.muted} loop={block.loop} autoPlay={block.autoplay}
      poster={block.thumbnailUrl} className={cn("aspect-video w-full bg-black", roundedCls)} />;
  }
  ...
  if (block.provider === "youtube") {
    ...
    if (!id) return <div className="rounded-md border border-dashed p-3 text-center text-xs text-muted-foreground">Invalid YouTube URL</div>;
    src = `https://www.youtube.com/embed/${id}?${params.toString()}`;
  } else {
    const id = extractVimeoId(block.url);
    if (!id) return <div className="rounded-md border border-dashed p-3 text-center text-xs text-muted-foreground">Invalid Vimeo URL</div>;
  ...
  return (
    <div className={cn("aspect-video w-full overflow-hidden bg-black", roundedCls)}>
      <iframe src={src} className="h-full w-full" allow="autoplay; encrypted-media; picture-in-picture"
...
function GalleryRender({ block }: { block: GalleryBlock }) {
  ...
  const rounded = block.rounded === "lg" ? "rounded-xl" : block.rounded === "md" ? "rounded-lg"
    : block.rounded === "sm" ? "rounded" : "rounded-none";
  const gap = block.gap === "lg" ? "gap-4" : block.gap === "sm" ? "gap-1" : "gap-2";
  if (block.images.length === 0) {
    return <div className="rounded-md border border-dashed p-4 text-center text-xs text-muted-foreground">Add gallery images</div>;
  }
  ...
  return (
    <div className="rounded-lg border bg-muted/30 p-4 text-center">
      ...
      <div className="mt-1 text-[11px] text-muted-foreground">
        {block.handle ? `@${block.handle.replace(/^@/, "")}` : "Connect an account to load the feed"}
      </div>
  ...
  return (
    <div className="space-y-3">
      ...
      {block.items.length === 0 ? (
        <div className="rounded-md border border-dashed p-3 text-center text-xs text-muted-foreground">No testimonials</div>
      ) : block.items.map((t) => (
        <div key={t.id} className="rounded-xl border bg-card p-3">
          <div className="flex items-center gap-2">
            ...
            <div className="min-w-0">
              ...
              {t.role && <div className="truncate text-[10px] text-muted-foreground">{t.role}</div>}
            </div>
            {typeof t.rating === "number" && (
              <div className="ml-auto flex">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star key={i} className={cn("h-3 w-3", i < (t.rating ?? 0) ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground/40")} />
                ))}
              </div>
            )}
          </div>
...
function FaqRender({ block }: { block: FaqBlock }) {
  return (
    <div className="space-y-2">
      ...
      {block.items.length === 0 ? (
        <div className="rounded-md border border-dashed p-3 text-center text-xs text-muted-foreground">No questions</div>
      ) : (
        <Accordion type="single" collapsible className="w-full">
          {block.items.map((it) => (
            <AccordionItem key={it.id} value={it.id}>
              <AccordionTrigger className="text-left text-sm">{it.question || "Question"}</AccordionTrigger>
              <AccordionContent className="text-xs text-muted-foreground">{it.answer}</AccordionContent>
            </AccordionItem>
          ))}
        </Accordion>
      )}
...
function CountdownRender({ block }: { block: CountdownBlock }) {
  ...
  if (isNaN(target)) {
    return <div className="rounded-md border border-dashed p-3 text-center text-xs text-muted-foreground">Set a target date</div>;
  }
  if (diff <= 0) {
    return (
...
function MapRender({ block }: { block: MapBlock }) {
  if (!block.mapUrl) {
    return <div className="rounded-md border border-dashed p-4 text-center text-xs text-muted-foreground">Add a Google Maps URL</div>;
  }
  const isEmbed = /google\.com\/maps\/embed/.test(block.mapUrl);
  return (
    <div className="overflow-hidden rounded-xl border">
      {isEmbed ? (
        <iframe src={block.mapUrl} className="h-48 w-full" loading="lazy" title="Map"
      ...
      <div className="flex items-center justify-between gap-3 p-3">
        <div className="min-w-0">
          {block.locationName && <div className="truncate text-sm font-medium">{block.locationName}</div>}
          {block.address && <div className="truncate text-[11px] text-muted-foreground">{block.address}</div>}
        </div>
        <div className="shrink-0 rounded-full bg-foreground px-3 py-1 text-[11px] font-medium text-background">Open</div>
      </div>
    </div>
  );
}
...
function FileRender({ block }: { block: FileBlock }) {
  const Icon = block.fileKind === "zip" ? FileArchive
    : block.fileKind === "image" ? ImageIcon
    : block.fileKind === "docx" ? FileText
    : block.fileKind === "pdf" ? FileText : FileIcon;
  return (
    <div className="flex items-center gap-3 rounded-xl border bg-card p-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        {block.sizeLabel && <div className="text-[11px] text-muted-foreground">{block.sizeLabel}</div>}
      </div>
...
const EMBED_MATCHERS: Record<EmbedBlock["provider"], (u: string) => string | null> = {
  spotify: (u) => /open\.spotify\.com\//.test(u)
    ? u.replace("open.spotify.com/", "open.spotify.com/embed/") : null,
  appleMusic: (u) => /music\.apple\.com\//.test(u)
    ? u.replace("music.apple.com", "embed.music.apple.com") : null,
  googleForms: (u) => /docs\.google\.com\/forms\//.test(u)
    ? (u.includes("?embedded=true") ? u : `${u}${u.includes("?") ? "&" : "?"}embedded=true`) : null,
  typeform: (u) => /typeform\.com\//.test(u) ? u : null,
  youtube: (u) => {
    const m = u.match(/(?:youtu\.be\/|v=|embed\/|shorts\/)([\w-]{6,})/);
    return m ? `https://www.youtube.com/embed/${m[1]}` : null;
  },
  loom: (u) => /loom\.com\//.test(u)
    ? u.replace("/share/", "/embed/") : null,
  figma: (u) => /figma\.com\/(file|proto|design)\//.test(u)
    ? `https://www.figma.com/embed?embed_host=share&url=${encodeURIComponent(u)}` : null,
  canva: (u) => /canva\.com\//.test(u)
    ? (u.endsWith("?embed") ? u : `${u}?embed`) : null,
  notion: (u) => /notion\.(so|site)\//.test(u) ? u : null,
};
function EmbedRender({ block }: { block: EmbedBlock }) {
  if (!block.url) {
    return <div className="rounded-md border border-dashed p-3 text-center text-xs text-muted-foreground">Add an embed URL</div>;
  }
  const src = EMBED_MATCHERS[block.provider]?.(block.url);
  if (!src) {
    return <div className="rounded-md border border-destructive/40 bg-destructive/5 p-3 text-center text-xs text-destructive">
      URL is not a valid {block.provider} link
    </div>;
  }
  return (
    <iframe src={src} className="w-full overflow-hidden rounded-xl border"
      style={{ height: `${block.height ?? 232}px` }}
      loading="lazy" allow="autoplay; encrypted-media; picture-in-picture"
      allowFullScreen title={`${block.provider} embed`} />
  );
}
