...
import { Sparkles, Zap, Globe2, ShieldCheck, Palette, BarChart3, LayoutTemplate, CheckCircle2, TrendingUp, MessageSquare, ShoppingBag, Instagram, Twitter, Linkedin, Youtube, X, Calendar, CreditCard, Plus } from "lucide-react";
...
function NumberTicker({ value, duration = 2000 }) {
...
export function LandingPage() {
    ...
    useEffect(() => {
        ...
        const handleScroll = () => {
            ...
            if (children.length === 0)
                return;
            const cardWidth = children[0].offsetWidth + 16; // 16 is gap-4
    ...
    return (<main id="hero" className="min-h-screen bg-[#090B18] text-white selection:bg-[#FF6A3D]/30">
      <Navbar />
      ...
      <section className="relative pt-32 pb-20 overflow-hidden lg:pt-48 lg:pb-40">
        ...
        ].map((badge, i) => (<div key={i} className="flex items-center gap-2 text-[13px] font-medium text-[#B9C0D4]/80">
        ...
        ].map((btn, i) => (<div key={i} className={cn("w-full h-12 rounded-2xl flex items-center justify-center text-sm font-bold border border-white/10 shadow-lg", btn.color === "#12152A" ? "bg-[#12152A] text-white" : "bg-[#FF2DAA] text-white")}>
                        {btn.label}
                      </div>))}
                  </div>
      ...
      <section id="features" className="py-24 sm:py-32">
        <div className="container mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {FEATURES.map((f, i) => (<div key={i} className="group p-6 sm:p-8 rounded-[22px] border border-white/5 bg-[#12152A] hover:border-[#FF6A3D]/30 transition-all duration-300 flex flex-col h-full">
      ...
      <section id="features-bento" className="py-24 sm:py-32 bg-[#090B18] relative overflow-hidden">
            ...
            <div className="md:col-span-8 md:row-span-2 group relative p-6 sm:p-8 rounded-[32px] border border-white/5 bg-white/[0.02] hover:border-[#FF2DAA]/30 transition-all duration-500 flex flex-col h-full">
              ...
              <div className="relative z-10 flex flex-col h-full">
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  {["Physical + Digital", "Inventory", "Discounts", "0% Fee"].map((tag) => (<span key={tag} className="px-4 py-2 rounded-full bg-white/5 border border-white/10 text-[10px] sm:text-xs font-bold text-[#B9C0D4]">
                      {tag}
                    </span>))}
                </div>
              </div>
            ...
            <div className="md:col-span-4 md:row-span-2 group relative p-6 sm:p-8 rounded-[32px] border border-white/5 bg-white/[0.02] hover:border-[#FF7A45]/30 transition-all duration-500 flex flex-col h-full">
              <div className="relative z-10 flex flex-col h-full">
                ...
                <div className="mt-auto space-y-3">
                  {["Razorpay", "Cashfree", "PayU", "Offline UPI"].map((item) => (<div key={item} className="flex items-center gap-3 text-sm text-[#B9C0D4]">
            ...
            <div className="md:col-span-4 md:row-span-2 group relative p-6 sm:p-8 rounded-[32px] border border-white/5 bg-white/[0.02] hover:border-[#FF4D8D]/30 transition-all duration-500 flex flex-col h-full">
              <div className="relative z-10 flex flex-col h-full">
                ...
                <ul className="space-y-2 mt-auto">
                  {["Time Slots", "Auto Confirm", "WhatsApp Alerts"].map((f) => (<li key={f} className="flex items-center gap-2 text-[10px] sm:text-xs font-bold text-[#B9C0D4]/70">
            ...
            <div className="md:col-span-8 md:row-span-2 group relative p-6 sm:p-8 rounded-[32px] border border-white/5 bg-white/[0.02] hover:border-purple-500/30 transition-all duration-500 flex flex-col h-full">
              <div className="relative z-10 flex flex-col h-full">
                ...
                <div className="mt-auto grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {["Bio Gen", "CTA Gen", "Content Gen", "Headline Gen"].map((tool) => (<div key={tool} className="p-3 rounded-xl bg-white/5 border border-white/10 text-center text-[10px] sm:text-xs font-medium">
                      {tool}
                    </div>))}
                </div>
              </div>
        ...
        ].map((stat) => (<div key={stat.label} className="group/kpi p-3 sm:p-5 rounded-2xl bg-[#1A1C2E]/80 border border-white/10 text-center backdrop-blur-md shadow-xl transition-all duration-300 active:scale-[0.98] flex flex-col items-center justify-center min-h-[90px] w-[calc(50%-6px)] md:w-[calc(25%-12px)]">
        ...
        ].map((stat, i) => (<div key={i} className="group">
      ...
      <section id="pricing" className="py-24 sm:py-32 relative">
        <div className="container mx-auto px-6">
          ...
          <div className="flex justify-center items-center gap-4 mb-16">
            ...
            <button onClick={() => setBillingCycle(billingCycle === 'monthly' ? 'yearly' : 'monthly')} className="relative w-14 h-7 rounded-full bg-white/10 border border-white/10 p-1 transition-all duration-300">
              <div className={cn("w-5 h-5 rounded-full bg-gradient-to-r from-[#FF2DAA] to-[#FF7A45] transition-all duration-300 transform shadow-lg", billingCycle === 'yearly' ? "translate-x-7" : "translate-x-0")}/>
            </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-7xl mx-auto mb-32 items-stretch">
            {PRICING.map((p, i) => (<div key={i} className={cn("relative p-8 sm:p-10 rounded-[32px] border transition-all duration-500 flex flex-col group hover:shadow-[0_0_50px_-12px_rgba(255,45,170,0.3)] hover:-translate-y-2 h-full", p.popular
                ? "bg-white/[0.04] border-[#FF2DAA]/30 backdrop-blur-xl shadow-2xl z-10"
                : "bg-white/[0.02] border-white/5 backdrop-blur-md")}>
                {p.popular && (<div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-gradient-to-r from-[#FF2DAA] to-[#FF7A45] text-white text-[10px] font-bold px-5 py-2 rounded-full uppercase tracking-[0.2em] shadow-lg shadow-[#FF2DAA]/20">
                    Most Popular
                  </div>)}
                
                <div className="mb-8">
                  <h3 className={cn("text-lg font-bold mb-4 tracking-widest", p.popular ? "text-[#FF2DAA]" : "text-[#B9C0D4]")}>{p.name}</h3>
                  ...
                  {billingCycle === 'yearly' && p.name !== 'FREE' && (<div className="mt-2 text-[#FF2DAA] text-xs font-bold flex items-center gap-1.5">
                ...
                <ul className="space-y-4 mb-10 flex-1">
                  {p.features.map((f, j) => (<li key={j} className="flex items-start gap-3 text-sm text-[#B9C0D4] group/item">
                      <div className="mt-0.5 rounded-full p-0.5 bg-gradient-to-br from-[#FF2DAA]/20 to-[#FF7A45]/20">
                        <CheckCircle2 className={cn("h-4 w-4", p.popular ? "text-[#FF2DAA]" : "text-[#FF7A45]")}/>
                      </div>
                      <span className="group-hover/item:text-white transition-colors">{f}</span>
                    </li>))}
                </ul>

                <CtaButton to="/auth" variant={p.popular ? "primary" : "secondary"} className={cn("w-full h-14 text-base font-bold transition-all duration-300", p.name === 'PRO' ? "bg-gradient-to-r from-[#FF2DAA] to-[#FF7A45] border-none text-white hover:opacity-90" : "bg-white/5 border-white/10 hover:bg-white/10")} showIcon={false}>
                  {p.cta}
                </CtaButton>
              </div>))}
          </div>
            ...
            <div className="overflow-x-auto rounded-3xl border border-white/5 bg-white/[0.02] backdrop-blur-md">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-white/5">
                  {COMPARISON_FEATURES.map((feature, idx) => (<tr key={idx} className="group hover:bg-white/[0.01] transition-colors">
                      <td className="p-6 text-sm font-medium text-[#B9C0D4] group-hover:text-white transition-colors">{feature.name}</td>
                      <td className="p-6 text-center">
                        {typeof feature.free === 'string' ? (<span className="text-xs font-bold text-white/40">{feature.free}</span>) : feature.free ? (<CheckCircle2 className="h-5 w-5 text-white/20 mx-auto"/>) : (<X className="h-5 w-5 text-white/5 mx-auto"/>)}
                      </td>
                      <td className="p-6 text-center">
                        {typeof feature.starter === 'string' ? (<span className="text-xs font-bold text-[#FF7A45]">{feature.starter}</span>) : feature.starter ? (<CheckCircle2 className="h-5 w-5 text-[#FF7A45] mx-auto"/>) : (<X className="h-5 w-5 text-white/5 mx-auto"/>)}
                      </td>
                      <td className="p-6 text-center bg-[#FF2DAA]/5">
                        {typeof feature.pro === 'string' ? (<span className="text-xs font-bold text-[#FF2DAA]">{feature.pro}</span>) : feature.pro ? (<div className="relative inline-block group/check">
                          ...
                          </div>) : (<X className="h-5 w-5 text-white/5 mx-auto"/>)}
                      </td>
                    </tr>))}
                </tbody>
              </table>
      ...
      <section className="py-24 sm:py-32 relative overflow-hidden bg-[#090B18]">
        ...
        <div className="container mx-auto px-6 relative z-10 text-center">
          ...
          <div className="relative h-[400px] sm:h-[600px] w-full flex items-center justify-center mb-24 perspective-1000">
        ...
        ].map((item, i) => (<div key={i} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" style={{
                animation: `orbit ${item.speed}s linear infinite`,
                animationDelay: `-${item.delay}s`,
                width: `${item.radius * 2}px`,
                height: `${item.radius * 2}px`,
            }}>
                  <div className={cn("absolute top-0 left-1/2 -translate-x-1/2 p-3 rounded-xl bg-white/5 border border-white/10 backdrop-blur-md pointer-events-auto cursor-pointer transition-all duration-300 hover:scale-125 hover:border-white/30", item.highlight && "ring-2 ring-[#0077B5] ring-offset-4 ring-offset-[#090B18]")} style={{
            ...
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
              {[...Array(20)].map((_, i) => (<div key={i} className="absolute w-1 h-1 bg-white/20 rounded-full animate-pulse" style={{
                top: `${Math.random() * 100}%`,
        ...
        ].map((feature) => (<div key={feature} className="p-4 rounded-2xl bg-white/[0.02] border border-white/5 text-[11px] font-bold text-[#B9C0D4] uppercase tracking-wider hover:bg-white/[0.04] hover:border-white/10 transition-colors">
                {feature}
              </div>))}
          </div>
        ...
        ].map((badge) => (<span key={badge} className="px-4 py-2 rounded-lg bg-white/5 border border-white/10 text-xs font-medium text-[#B9C0D4]">
                      {badge}
                    </span>))}
                </div>
      ...
      <section id="faq" className="py-24 sm:py-32">
        <div className="container mx-auto px-6 max-w-3xl">
          ...
          <Accordion type="single" collapsible className="w-full space-y-4">
            {FAQS.map((faq, i) => (<AccordionItem key={i} value={`item-${i}`} className="border-white/5 bg-[#12152A] px-6 rounded-2xl">
        ...
        <div className="container mx-auto px-6 relative z-10 flex flex-col items-center text-center">
          <div className="motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-8 duration-1000">
            ...
            <div className="flex flex-col sm:flex-row min-[480px]:flex-wrap items-center justify-center gap-4 mt-7">
              <CtaButton to="/auth" showIcon={false} className="min-[480px]:flex-1 min-[480px]:min-w-[220px]">
                🎉 Start 3-Day Free Trial
              </CtaButton>
              <CtaButton to="/auth" variant="secondary" showIcon={false} className="min-[480px]:flex-1 min-[480px]:min-w-[220px]">
                ▶ Watch Live Demo
